...
local AddonMeta = {
	...
	GetVar = function(self,varid) 
		return self.data.shared_vars[varid] or self.data.vars[varid] 
	end,
	SetVar = function(self,varid,value)
...
if (CLIENT) then
	...
	function AddonMeta:DownloadMaterial(url, on_succ, on_err, retry_count)
	...
	function AddonMeta:ClearDownloadCache()
		esclib:ClearDownloadCache(self.info.uid)
	end
	...
	function AddonMeta:RequestServerSettings(callback)
	...
	net.Receive("esclib.SendServerConfig", function(len)
		...
		if istable(c_json) and not table.IsEmpty(c_json) then
			table.Empty(addon.data["server_settings"])
