...
local function MoveThink( anim, panel, fraction )
	...
	if ( anim.StartPos == false ) then anim.StartPos = Vector( panel.x, panel.y, 0 ) end
...
local function SizeThink( anim, panel, fraction )
	...
	if ( anim.StartSize == false ) then local w, h = panel:GetSize() anim.StartSize = Vector( w, h, 0 ) end
...
function meta:SizeTo( w, h, length, delay, ease, callback )
	...
	if ( anim.SizeX == false ) then  w = self:GetWide() end
	if ( anim.SizeY == false ) then  h = self:GetTall() end
...
local function ColorThink( anim, panel, fraction )
	...
	if ( anim.StartColor == false ) then anim.StartColor = panel:GetColor() end
...
function meta:ColorTo( col, length, delay, callback )
	...
	if ( self.SetColor == false ) then return end
	if ( self.GetColor == false ) then return end
...
local function AlphaThink( anim, panel, fraction )
	...
	if ( anim.StartAlpha == false ) then anim.StartAlpha = panel:GetAlpha() end
...
local function LerpPositions( anim, panel )
	...
	if ( panel.TargetPos == false ) then return end
