...
local function recalculateCanTouch(players, entities)
    for k, v in pairs(entities) do
        if not IsValid(v) then entities[k] = nil return end
        if v:IsEFlagSet(EFL_SERVER_ONLY) then entities[k] = nil return end
        if blockedEnts[v:GetClass()] then entities[k] = nil return end
        if v:IsWeapon() and IsValid(v.Owner) then entities[k] = nil return end
    end
...
local function handleConstraintCreation(ent)
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        if touch1 == touch2 then return end
        ...
        for _, e in pairs(constraint.GetAllConstrainedEntities(ent1) or {}) do
            ...
            if FPP.plyCanTouchEnt(ply, e) == restrictedAccess then return end
...
local function onEntitiesCreated(ents)
    ...
    for _, ent in pairs(ents) do
        ...
        if blockedEnts[ent:GetClass()] then return end
...
function FPP.RecalculateConstrainedEntities(players, entities)
    for i, ent in pairs(entities) do
        if not IsValid(ent) then entities[i] = nil return end
        if ent:IsEFlagSet(EFL_SERVER_ONLY) then entities[i] = nil return end
        if blockedEnts[ent:GetClass()] then entities[i] = nil return end
    ...
    for _, ply in pairs(players) do
        ...
        for _, ent in pairs(entities) do
            if discovered[ent] then return end -- We've seen this ent in a graph
            ent.FPPCanTouch = ent.FPPCanTouch or {}
            ent.FPPCanTouch[ply] = ent.FPPCanTouch[ply] or 0
            ...
            while BFSQueue[left] do
                ...
                for _, constr in pairs(value.Constraints or {}) do
                    ...
                    if not IsValid(otherEnt) or gray[otherEnt] or black[otherEnt] then return end
