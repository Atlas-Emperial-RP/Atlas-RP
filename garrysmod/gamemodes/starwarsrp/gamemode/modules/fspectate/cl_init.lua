...
local function findNearestObject()
    ...
    for _, ply in ipairs(player.GetAll()) do
        ...
        if dot < 0.97 then return end
        -- not a better alternative
        if dot < foundDot then return end
...
local function specThink()
    ...
    for i = 0, #pls - 1 do
        ...
        if not isRoaming and p == specEnt and not thirdperson then skip = skip + 3 return end
...
local function drawHelp()
    ...
    for i = 1, #pls do
        local ply = pls[i]
        if not IsValid(ply) then return end
        if not isRoaming and ply == specEnt then return end
