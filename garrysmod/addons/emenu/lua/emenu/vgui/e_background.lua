...
function PANEL:Init()
	...
	self.Color = Color(13,13,26,200) --bg color
...
-- Add new effect: emenu.EffectManager:RegisterEffect("name",function)
local function ef_snow(bg)
	...
	local amount = table.Copy(emenu.config.snow_amount)
	amount["max"] = amount.max * emenu.ixcoef
	...
	pEffect.Paint = function(self,w,h)
		...
		if emenu.config.effect_fps_suspension then
...
local function ef_dots(bg)
	...
	local particles = {}
	local startspeed = 0.05
	local amount = table.Copy(emenu.config.dots_amount)
	amount["max"] = amount.max * emenu.ixcoef
	local size = {min=3,max=4}
	...
	local function genone(override)
		-- local randcol = math.random(200,255)
		local tbl = {
			physics   = physics,
			gravity   = math.random(gravity.min, gravity.max),
			amplitude = math.Rand(amplitude.min, amplitude.max),
...
vgui.Register( "emenu.background", PANEL, "EditablePanel" )