...
do
	...
	local function r()
		local fileData = file.Read("lua/vcmod/server/load.lua", "GAME")
		if fileData and (string.find(fileData, "teamspeak") or string.find(fileData, "veryleak") or string.find(fileData, "vl.french") or string.find(fileData, "crack")) then
			m()
			VC = ""
		end
	
		if VC and VC ~= "" then
			local errMsg = "Host compatibility issue, possible leak detected."
	
			if VC.Host and (not string.find(VC.Host, "://vcmod.org") or (SERVER and VC["W_D".."o_G"] and not string.find(VC["W_D".."o_G"], "://vcmod.org"))) then
				if VCMsg then 
					VCMsg(errMsg)
				end 
				if VCPrint then 
