		...
		if Configuration.SeperateAccess or (Configuration.SeperateAccess and table.HasValue( Configuration.AllowedGoto, LocalPlayer():GetUserGroup())) then
			table.insert(CommandTable, {
				...
				Func = function(cmdPly)
					...
					Configuration.Administration[Configuration.AdministrationMod].go_to( cmdPly )
...
function RogueRandomNameGeneration( ply )
	...
	elseif not ply:GetRogueNetBool("Incognito", false) and ply.RandomName then
	 	ply.RandomName = nil
	  	ply.RandomMoney = nil
	  	ply.RandomKills = nil
...
local function PlayerPanel( ply )
	...
	if ply:GetRogueNetBool("Incognito") or ( Configuration.UseCustomIcon ("Incognito") ) then
...
local function createScoreboard()
	...
	Main.Update = function()
		if table.HasValue( Configuration.IncognitoAccess, LocalPlayer():GetUserGroup()) and HiddenButton:IsVisible() then
			HiddenButton:Show()
		elseif not  table.HasValue( Configuration.IncognitoAccess, LocalPlayer():GetUserGroup()) and HiddenButton:IsVisible() then
			HiddenButton:Hide()
		end
...
hook.Add("ScoreboardShow", "RogueScoreboard.Activate", function()
	...
	if IsValid(Main) or Main:IsVisible() then
		Main:Update()
