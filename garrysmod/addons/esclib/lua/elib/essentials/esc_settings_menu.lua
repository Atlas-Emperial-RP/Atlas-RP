...
function esclib:opensettings(naddon)
	...
	function settab:Paint(w,h)
		...
		draw.SimpleText(esclib.addon:GetName(), font, w-15, h-65, clr.frame.text, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
		draw.SimpleText(version, font, w-15, h-40, clr.frame.text, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
		draw.SimpleText("SVUID "..hash, font, w-15, h-15, clr.frame.text, TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
	...
	if esclib.addon:GetVar("debug") then
		...
		function dbg_btn:DoClick()
			...
			for id,font in pairs(fonts) do
				...
				font_pnl:SetSize(list:GetWide()-list:GetBorder()*2, math.max(texth+6,texth2+6)+20)
	...
	for uid,add in pairs(addons) do
		...
		local name_cut = esclib.util:TextCutAccurate(add.info.name, esclib:AdaptiveFont("esclib", 20, 500), addpan:GetWide()-20, "...")
		local edit_cut = esclib.util:TextCutAccurate(esclib.addon:Translate("button_Edit"), esclib:AdaptiveFont("esclib", 20, 500), addpan:GetWide()-20, "...")
		...
		local font_20 = esclib:AdaptiveFont("esclib", 20, 500)
		...
		function addpan:Paint(w,h)
			...
			--version
			esclib.draw:ShadowText(version, font_16, w-version_wide-15, 10, clr.default.white)
			...
			if hovered then
				...
				draw.SimpleText(edit_cut, font_20, w*0.5, w+(h-w)*0.5, clr.frame.text,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
			else
				draw.SimpleText(name_cut, font_20, 10, w+(h-w)*0.5, clr.frame.text,TEXT_ALIGN_LEFT,TEXT_ALIGN_CENTER)
			end
		...
		local function open_addon_edit()
			...
			realm:AddTab(esclib.addon:Translate("realm_Client",add:GetLanguage()), function(content)
				...
				local function check_saved(varid,val)
					if changed_vars[varid] ~= nil then
						if VarsIsEqual(add.data.vars[varid], val) then
				...
				clearcfg_button:eAddHint(clearcfg_button:GetButtonText(),esclib:AdaptiveFont("esclib", 24, 500),TEXT_ALIGN_CENTER,settab)
				...
				apply_button:SetButtonText(esclib.addon:Translate("phrase_Save", add:GetLanguage()))
				...
				apply_button:eAddHint(apply_button:GetButtonText(),esclib:AdaptiveFont("esclib", 24, 500),TEXT_ALIGN_CENTER,settab)
			...
			add:RequestServerSettings(function()
				local addon_sv_settings_copy = table.Copy(add.data.server_settings) or {}
				if table.IsEmpty(addon_sv_settings_copy) then return end

				realm:AddTab(esclib.addon:Translate("realm_Server",add:GetLanguage()), function(content)
					if not IsValid(content) then return end

					if table.IsEmpty(addon_sv_settings_copy) or not esclib:HasAdminAccess(LocalPlayer()) then
						...
						function content:Paint(w,h)
							draw.SimpleText(text, esclib:AdaptiveFont("esclib", 24, 500), w*0.5,h*0.5, clr.frame.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
						end
					else
						local changed_vars = {}
						local settings_changed = false 
						...
						--var manipulation
						local addon_settings_copy = table.Copy(addon_sv_settings_copy)
						...
						--useful functions
						local function check_saved(varid,val) --duplicate?
							if changed_vars[varid] ~= nil then
