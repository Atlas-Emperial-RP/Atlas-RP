...
function PANEL:Refresh()
    ...
    local values = PROJECT0.FUNC.GetChangedVariable( "CUSTOMISER", "Charms" ) or PROJECT0.CONFIGMETA.CUSTOMISER:GetConfigValue( "Charms" )

    local sortedCharms = {}
    for k, v in pairs( values ) do
        table.insert( sortedCharms, { PROJECT0.FUNC.GetRarityOrder( v.Rarity ), k } )
    end

    table.SortByMember( sortedCharms, 1 )

    local panelH = PROJECT0.FUNC.ScreenScale( 75 )
    local textMargin = 5+panelH+PROJECT0.UI.Margin15
    for k, v in ipairs( sortedCharms ) do
        local charmKey = v[2]
        local charmConfig = values[charmKey]

        local rarityName = PROJECT0.FUNC.GetRarityName( charmConfig.Rarity )
        local colors = PROJECT0.FUNC.GetRarityColor( charmConfig.Rarity )
        ...
        variablePanel.Paint = function( self2, w, h )
            ...
            draw.SimpleText( charmConfig.Name, "MontserratBold22", textMargin, h/2+1, PROJECT0.FUNC.GetSolidColor( colors ), 0, TEXT_ALIGN_BOTTOM )
        ...
        local modelPanel = vgui.Create( "pz_modelpanel", variablePanel )
        modelPanel:Dock( LEFT )
        modelPanel:SetWide( panelH-(2*PROJECT0.UI.Margin10) )
        modelPanel:DockMargin( 5+PROJECT0.UI.Margin10, PROJECT0.UI.Margin10, 0, PROJECT0.UI.Margin10 )
        ...
        editButton.DoClick = function()
            self:CreateConfigPopup( charmKey, values )
        end
    end
    ...
    addNewButton.DoClick = function()
        ...
        PROJECT0.FUNC.RequestConfigChange( "CUSTOMISER", "Charms", values )
        self:Refresh()
    end
end
...
function PANEL:CreateConfigPopup( valueKey, values )
    ...
    popup:SetInfo( "Charm Info", "Basic information and actions.", Material( "project0/icons/data.png" ) )

    popup:AddActionButton( "Delete", Material( "project0/icons/delete.png" ), function()
        PROJECT0.FUNC.DermaQuery( "Are you sure you want to delete this charm?", "CHARM DELETION", "Confirm", function()
            values[valueKey] = nil
            valueChanged = true
            popup:Close()
        end, "Cancel" )
    ...
    -- ITEM SLOT --
    local modelPanel = vgui.Create( "pz_modelpanel", popup )
    modelPanel:SetSize( PROJECT0.FUNC.Repeat( PROJECT0.FUNC.ScreenScale( 200 ), 2 ) )
    modelPanel:SetPos( popup.sectionMargin+popup.sectionWide/2-modelPanel:GetWide()/2, popup.header:GetTall()+(popup.mainPanel.targetH-popup.header:GetTall()-popup.infoBack:GetTall())/2-modelPanel:GetTall()/2 )
    modelPanel:SetFOV( 30 )
    modelPanel:ChangeModel( configItem.Model )

    -- NAME --
    local nameField = popup:AddField( "Name", "The name of the charm.", Material( "project0/icons/name_24.png" ) )
    ...
    -- RARITY --
    local rarityField = popup:AddField( "Rarity", "The rarity of the charm.", Material( "project0/icons/rarity.png", "noclamp smooth" ) )
    ...
    -- MODEL --
    local modelField = popup:AddField( "Model", "The model used for the charm.", Material( "project0/icons/image_24.png" ) )
    ...
    modelHint:Dock( TOP )
    modelHint:DockMargin( 0, margin10, 0, margin10 )
    ...
    modelEntry = vgui.Create( "pz_textentry", modelField )
    modelEntry:Dock( TOP )
    modelEntry:DockMargin( margin10, 0, margin10, 0 )
    modelEntry:SetTall( PROJECT0.FUNC.ScreenScale( 40 ) )
    modelEntry:SetBackColor( PROJECT0.FUNC.GetTheme( 1 ) )
    modelEntry:SetHighlightColor( PROJECT0.FUNC.GetTheme( 2, 25 ) )
    modelEntry:SetValue( configItem.Model )
    modelEntry:DisableShadows( true )
    modelEntry.OnEnter = function( self2 )
        ChangeItemVariable( "Model", self2:GetValue() )
        modelPanel:ChangeModel( configItem.Model )
    end

    modelField:SetExtraHeight( margin10+modelHint:GetTall()+margin10+modelEntry:GetTall()+margin10 )
end

vgui.Register( "pz_config_customiser_charms", PANEL, "pz_config_page_base" )