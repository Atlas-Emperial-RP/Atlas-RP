...
function PANEL:Init()
    ...
    for k, v in pairs( sortedConfig ) do
        ...
        for key, val in ipairs( configMeta:GetSortedVariables() ) do
            if( val.Type == PROJECT0.TYPE.Table ) then break end
...
function PANEL:Refresh()
    for k, v in pairs( self.pages ) do
        if( not v.Panel.Refresh ) then break end
        v.Panel:Refresh()
...
function PANEL:OpenPageByID( id, variableKey )
    for k, v in ipairs( self.pages ) do
        if( v.ConfigID ~= id or (variableKey and v.VariableKey ~= variableKey) ) then break end
