function PROJECT0.FUNC.DermaMessage( text, title, buttonText, buttonFunc )
    buttonText = buttonText or "OK"
    title = title or "MESSAGE"

	local popup = vgui.Create( "pz_popup_base" )
	...
	textArea.Paint = function( self2, w, h ) 
		draw.SimpleText( text, "MontserratMedium20", w/2, h/2, PROJECT0.FUNC.GetTheme( 4 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
    ...
    bottomButton.Paint = function( self2, w, h )
        ...
        draw.RoundedBox( 8, 0, 0, w, h, PROJECT0.FUNC.GetTheme( 3, alpha ) )

        draw.SimpleText( buttonText, "MontserratBold25", w/2, h/2-1, PROJECT0.FUNC.GetTheme( 4, 75+(180*(alpha/75)) ), 1, 1 )
    end
...
function PROJECT0.FUNC.DermaQuery( text, title, ... )
    ...
    local function createButton( text, func )
        ...
        button.Paint = function( self2, w, h )
            ...
            draw.SimpleText( text, "MontserratBold25", w/2, h/2-1, PROJECT0.FUNC.GetTheme( 4, 75+(180*(alpha/75)) ), 1, 1 )
        end
...
function PROJECT0.FUNC.DermaStringRequest( text, title, default, confirmText, confirmFunc, cancelText, cancelFunc )
    default = default or ""
    ...
    local textEntry = vgui.Create( "pz_textentry", popup )
	textEntry:Dock( TOP )
...
function PROJECT0.FUNC.DermaNumberRequest( text, title, default, confirmText, confirmFunc, cancelText, cancelFunc )
    ...
    local numberWang = vgui.Create( "pz_numberwang", popup )
	numberWang:Dock( TOP )
...
-- TODO: Update derma request UI
function PROJECT0.FUNC.DermaComboRequest( text, title, options, default, searchSelect, confirmText, confirmFunc, cancelText, cancelFunc )
    default = default or ""
    confirmText = confirmText or "OK"
    cancelText = cancelText or "Cancel"
    title = title or "REQUEST"
    ...
    local comboEntry = vgui.Create( searchSelect and "pz_combosearch" or "pz_combo", popup )
	comboEntry:Dock( TOP )
    ...
    local function createButton( text, func )
        ...
        button.Paint = function( self2, w, h )
            ...
            surface.SetDrawColor( PROJECT0.FUNC.GetTheme( 3, alpha ) )
            surface.DrawRect( 0, 0, w, h )
    ...
    createButton( confirmText, function()
		...
		if( value and data ) then
            ...
            PROJECT0.FUNC.CreateNotification( "MENU ERROR", "You need to select a value!", "error" )
		end
    end )
    createButton( cancelText, cancelFunc )
...
function PROJECT0.FUNC.DermaColorRequest( text, title, default, confirmText, confirmFunc, cancelText, cancelFunc )
...
function PROJECT0.FUNC.DermaProgressBar( title, textFunc, percentFunc, cancelFunc )
	local popup = vgui.Create( "pz_popup_base" )
	popup:SetHeader( title )
	...
	textArea.Paint = function( self2, w, h ) 
		draw.SimpleText( textFunc(), "MontserratMedium20", w/2, h/2, PROJECT0.FUNC.GetTheme( 4 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
	...
	progressArea.Paint = function( self2, w, h )
        ...
        PROJECT0.FUNC.DrawRoundedMask( 8, 0, 0, w, h, function()
            surface.SetDrawColor( PROJECT0.FUNC.GetTheme( 3, 100 ) )
            surface.DrawRect( 0, 0, w*percent, h )
        end )

		draw.SimpleText( math.floor( percent*100 ) .. "%", "MontserratBold25", w/2, h/2-1, PROJECT0.FUNC.GetTheme( 4 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
	end
    ...
    if( cancelFunc ) then
        ...
        bottomButton.Paint = function( self2, w, h )
            ...
            draw.SimpleText( "Cancel", "MontserratBold25", w/2, h/2-1, PROJECT0.FUNC.GetTheme( 4, 75+(180*(alpha/75)) ), 1, 1 )
        end
        bottomButton.DoClick = function()
            popup:Close()
        end
