...
hook.Add("PlayerSay", "NCS_LOADOUTS_MainCommandRun", function(P, T)
    if string.lower(T) == NCS_LOADOUTS.CONFIG.menucommand then
        ...
        if ( NCS_LOADOUTS.CONFIG.accessoption == 3 ) then SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_accessDisabled")) return "" end
    ...
    elseif string.lower(T) == NCS_LOADOUTS.CONFIG.savevendorcommand then
        NCS_LOADOUTS.IsAdmin(P, function(ACCESS)
            ...
            SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_savedVendors", {COUNT, game.GetMap()}))
        end )
...
local function UnequipLoadout(P, UID)
    ...
    if !TAB then
        SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_noEquipAccess"))
    else
        ...
        if TAB.weps then
            ...
            if RPExtraTeams and RPExtraTeams[pTeam] and RPExtraTeams[pTeam].weapons then
                for k, v in ipairs(RPExtraTeams[pTeam].weapons) do
                    job_Weapons[v] = true
                end
            end

            for k, v in pairs(TAB.weps) do
                if job_Weapons[k] then continue end
...
net.Receive("NCS_LOADOUTS_DelLoadout", function(_, P)
    NCS_LOADOUTS.IsAdmin(P, function(ACCESS)
        if !ACCESS then SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_noPermsAccess")) return end
...
hook.Add("playerGetSalary", "NCS_LOADOUTS_SalaryModifier", function(P, AMOUNT)
    ...
    if loadoutData and loadoutData.salary then
        ...
        local lang = NCS_SHARED.GetLang(nil, "LOADOUTS_loadoutSalaryExec", { DarkRP.formatMoney(AMOUNT), DarkRP.formatMoney(salaryModifier), loadoutData.name})
...
net.Receive("NCS_LOADOUTS_EquipLoadout", function(_, P)
    ...
    if P_DATA.antiSpamDelay and P_DATA.antiSpamDelay > CurTime() then SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_antiSpamDelay")) return end
    ...
    if !TAB then
        ...
        SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_equipSuccess", {(TAB.name or UID)}))
...
net.Receive("NCS_LOADOUTS_UpdateSettings", function(_, P)
    NCS_LOADOUTS.IsAdmin(P, function(ACCESS)
        ...
        SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_savedSettings"))
    end )
end )
...
net.Receive("NCS_LOADOUTS_GivePlayerCert", function(_, P)
    NCS_LOADOUTS.IsAdmin(P, function(ACCESS)
        ...
        if PID and IsValid(Player(PID)) then
            ...
            if GIVE then
                if ( P_R ~= P ) then
                    SendNotification(P_R, NCS_SHARED.GetLang(nil, "LOADOUTS_givenCertification", {(C_NAME or "N/A"), P:Name()}))
                    SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_youGivenCert", {(C_NAME or "N/A"), P_R:Name()}))
                else
                    SendNotification(P_R, NCS_SHARED.GetLang(nil, "LOADOUTS_selfGivenCertification", {(C_NAME or "N/A")}))
                end

                hook.Run("NCS_LOADOUTS_PlayerGiveCert", P_R, P, CID)
            else
                if ( P_R ~= P ) then
                    SendNotification(P_R, NCS_SHARED.GetLang(nil, "LOADOUTS_takenCertification", {(C_NAME or "N/A"), P:Name()}))
                    SendNotification(P, NCS_SHARED.GetLang(nil, "LOADOUTS_youTakenCert", {(C_NAME or "N/A"), P_R:Name()}))
                else
                    SendNotification(P_R, NCS_SHARED.GetLang(nil, "LOADOUTS_selfTakenCertification", {(C_NAME or "N/A")}))
                end
...
hook.Add("NCS_SHARED_CharacterLoaded", "NCS_LOADOUTS_OnCharacterLoaded", function(P)
    NCS_LOADOUTS_ApplyPlayerData(P)
end )

hook.Add("NCS_SHARED_CharacterDeleted", "NCS_LOADOUTS_CharacterDeleted", function(P, SLOT)
    local q = NCS_LOADOUTS_Mysql:Delete("NCS_LOADOUTS_V1")
        q:Where("PlayerID", (P:SteamID64()..SLOT))
    q:Execute()
end )   
