...
net.Receive("NCS_TRANSPORT_UpdateLocation", function(_, P)
    NCS_TRANSPORT.IsAdmin(P, function(ACCESS)
        ...
        if newData.uid and NCS_TRANSPORT.LOCATIONS[newData.uid] then
            ...
            SendNotification(P, NCS_SHARED.GetLang(nil, "TRANSPORT_locationUpdated", {newData.name}))
        else
            newData.uid = os.time()

            NCS_TRANSPORT.LOCATIONS[newData.uid] = newData

            SendNotification(P, NCS_SHARED.GetLang(nil, "TRANSPORT_locationAdded", {newData.name}))
        end
...
net.Receive("NCS_TRANSPORT_TravelSuccess", function(_, P)
    ...
    if !NCS_SHARED.CanAfford(P, nil, finalPrice) then return end

    NCS_SHARED.AddMoney(P, nil, -finalPrice)
    ...
    timer.Simple(2, function()
        ...
        SendNotification(P, NCS_SHARED.GetLang(nil, "TRANSPORT_youTraveled", {CFG[UID].name, NCS_SHARED.FormatMoney(nil, finalPrice)}))
    end )
end )
...
hook.Add("PlayerSay", "NCS_TRANSPORT_MainCommand", function(P, T)
    if string.lower(T) == string.lower(NCS_TRANSPORT.CONFIG.vendorSave) then
        NCS_TRANSPORT.IsAdmin(P, function(ACCESS)
            ...
            SendNotification(P, NCS_SHARED.GetLang(nil, "TRANSPORT_savedVendors", {COUNT, game.GetMap()}))
        end )
